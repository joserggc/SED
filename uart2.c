...
int uart2_cfg(int baudrate) {
  int ret;
  
  LPC_PINCON->PINSEL0 &= ~((3<<20) | (3<<22));
  LPC_PINCON->PINSEL0 |=  (3<<20)   | // Setup P0.10 as TXD2
                          (3<<22);    // Setup P0.11 as RXD2 

  LPC_UART2->LCR = CHAR_8_BITS | STOP_1_BIT;  // Set 8N1 mode (8 bits, no parity, 1 stop bit)
  ret = uart2_set_baudrate(baudrate);         // Set the baud rate

  LPC_UART2->IER = UART_THRE_IE | UART_RBR_IE;  // Enable UART TX and RX interrupt (for LPC17xx UART)
  NVIC_EnableIRQ(UART2_IRQn);                   // Enable the UART interrupt (for Cortex-CM3 NVIC)
  return ret;
}

int uart2_send_string(const char *txt) {
  int ret = -1;
  if(!tx_busy) {
    tx_ptr = txt;
    LPC_UART2->THR = *tx_ptr++;    
    tx_busy = 1;
    ret = 0;
  }
  return ret;
...
int uart2_get_tx_busy() {
  return tx_busy;
}

int uart2_recv_string(char *txt) {
  int ret = -1;
  if(!rx_busy) {
    rx_ptr = txt;
    rx_busy = 1;
